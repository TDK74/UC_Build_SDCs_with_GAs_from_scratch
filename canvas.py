...
class Canvas(Window):
    ...
    def simulate_generation(self, networks, simulation_round):
        self.car_sprites = []
        self.max_speed_on_track = 0.0
        self.hud = Hud(simulation_round, networks[0].dimensions, self.overlay_batch)

        for network in networks:
            self.car_sprites.append(Car(network, self.track, random.choice(self.car_images),
                                        self.cars_batch))
        ...
        print(f"🏎️  Max speed on Track {self.track}: {self.max_speed_on_track:.2f}")
    ...
    def update(self, delta_time):
        for car_sprite in self.car_sprites:
            ...
            if car_sprite.is_running:
                ...
                if car_sprite.last_checkpoint_passed >= 1:
                    car_sprite.avoid_cars([c for c in self.car_sprites if c.is_running])

        running_cars = [c for c in self.car_sprites if c.is_running]
        self.population_alive = len(running_cars)

        if self.population_alive > 0:
            current_speed = running_cars[0].speed
            self.max_speed_on_track = max(self.max_speed_on_track, current_speed)
            self.hud.update(running_cars[0].network, self.population_alive, self.population_total,
                            current_speed, self.max_speed_on_track)

        else:
            if any(car.network.has_reached_goal for car in self.car_sprites):
                print("✅ All cars have completed the simulation.")
            else:
                pass # print("💥 All cars are out! End of simulation.")
